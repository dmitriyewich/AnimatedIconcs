...
if doesFileExist("moonloader/AnimatedIconcs/AnimatedIconcs.json") then
    local f = io.open("moonloader/AnimatedIconcs/AnimatedIconcs.json")
...
function main()
	...
	if doesFileExist("moonloader/AnimatedIconcs/Anim/outline_anim.txd") then
		if mad.get_txd('txd_outline_anim') ~= nil then
			...
			txd_outline_anim = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Anim//outline_anim.txd', 'txd_outline_anim')
		end
	...
	---------------------txd_anim--------------------txd_anim----------
	local mask = getWorkingDirectory() .. "//AnimatedIconcs//Anim//*.txd"
	...
	while handle and file do
		if file ~= "." and file ~= ".." and file ~= "outline_anim.txd" then
			...
			if doesFileExist("moonloader/AnimatedIconcs/Anim/"..file) and checktable(id_gun, tdx_gsub) then
				if mad.get_txd(tdx_gsub) ~= nil then -- Проверяет наличие txd в памяти, костыль на случай перезапуска скрипта(ов)
					...
					txd_file = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Anim//'..file, tdx_gsub)
				end
	...
	if time ~= nil then
	  files[string.format("%s/AnimatedIconcs/AnimatedIconcs.json",getWorkingDirectory())] = time
	end
